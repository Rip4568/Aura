...
def _seeder(name: str) -> str:
    p = _pascal(name)
    return f'''\
"""Seeder for the {p} resources.
...
# Import your models or repositories here:
# from app.models import {p}


@injectable
class {p}Seeder(Seeder):
    async def run(self) -> None:
        # Define your seeding logic here, e.g.:
        # item = {p}(name="Sample Data")
...
@app.command("seeder")
def generate_seeder(
    name:   str  = typer.Argument(..., help="Model/Seeder name (e.g. 'user', 'product')"),
    output: str  = typer.Option("database/seeders", "--out", "-o", help="Output directory"),
    force:  bool = typer.Option(False, "--force", "-f", help="Overwrite if exists"),
) -> None:
    """Generate a Database Seeder skeleton."""
    s = _snake(name)
    path = Path(output) / f"{s}_seeder.py"
    console.print(f"\n[bold]Generating seeder for [cyan]{_pascal(name)}[/]...[/]\n")
    _write(path, _seeder(name), force)
    console.print(f"\n[bold green]✓ Seeder written to {path}[/]")
...
def _list_commands(ctx: typer.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        ...
        table.add_row("seeder <name>",     "Seeder class skeleton for database seeding")
