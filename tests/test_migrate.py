...
from aura.orm.migrations import autoload_project_models, generate_env_py
...
class TestInitCommand:
    ...
    def test_init_without_models_generates_placeholder(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert "autoload_project_models" in env_content
        finally:
            os.chdir(orig)

    def test_init_env_py_is_utf8_and_compilable(self, tmp_path: Path) -> None:
        """Regression: Windows cp1252 write broke Alembic env.py (em-dash -> \\x97)."""
        import os
        ...
        try:
            ...
            assert b"\x97" not in env_bytes
            env_content = env_bytes.decode("utf-8")
            compile(env_content, "env.py", "exec")
        finally:
            os.chdir(orig)
...
class TestGenerateEnvPy:
    ...
    def test_without_model_import_has_autoload(self) -> None:
        content = generate_env_py(Path("migrations"), None)
        assert "autoload_project_models" in content
        assert "AuraModel.metadata" in content
    ...
    def test_generated_docstring_is_ascii_only(self) -> None:
        content = generate_env_py(Path("migrations"), None)
        assert "Alembic env.py - generated by Aura CLI." in content
